...
async function ensureEmployee(
  ...
  if (normalized) {
    ...
    if (userByPhone?.employee) {
      const val = { employeeId: userByPhone.employee.id, userId: userByPhone.id };
      cache.set(normalized, val);
      cache.set(normalizedName, val);
      return val;
    }
  }

  const byName = await tx.employee.findFirst({
    where: {
      user: {
        name: {
          equals: name.trim(),
  ...
  if (byName?.user) {
    const val = { employeeId: byName.id, userId: byName.user.id };
    if (normalized) cache.set(normalized, val);
    cache.set(normalizedName, val);
    return val;
  }

  throw new Error(`Employee "${name.trim()}" does not exist. Import aborted.`);
}
