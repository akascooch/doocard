...
export class PushNotificationsService {
  ...
  constructor(private readonly prisma: PrismaService) {
    ...
    const vapidSubject =
      ...
      'mailto:admin@doocardbarbershop.com';
  ...
  private async sendNotification(subscription: any, payload: NotificationPayload, retryCount: number = 0) {
    ...
    try {
      ...
      if (isApplePush) {
        ...
        // NOTE: do NOT set options.topic here. Topic must be URL-safe Base64
        // (A-Za-z0-9-_ only); a dotted value like 'com.example.web' makes
        // web-push throw 'Unsupported characters set...' before sending.
