...
export class PushNotificationsController {
  ...
  async subscribe(@Body() dto: SubscribeDto, @Req() req: any) {
    console.log('📥 Push subscription request from user:', req.user.id, req.user.phone);
    console.log('Endpoint:', dto.endpoint.substring(0, 60) + '...');
    console.log('p256dh (first 20 chars):', dto.keys.p256dh.substring(0, 20));
    console.log('auth (first 20 chars):', dto.keys.auth.substring(0, 20));
  ...
  async sendTest(@Req() req: any) {
    console.log('🧪 Test notification request from user:', req.user.id, req.user.phone);
