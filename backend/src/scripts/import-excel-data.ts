...
// Utility: Find or create employee
async function findOrCreateEmployee(name: string): Promise<number> {
  if (!name || name.trim() === '') {
  ...
  console.log(`✅ Employee created: ${name} (id: ${employee.id}, phone: ${randomPhone})`);
  return employee.id;
}
...
async function importAppointments(filePath: string) {
  ...
  for (const row of rows) {
    try {
      ...
      const employeeId = await findOrCreateEmployee(row.employeeName);
