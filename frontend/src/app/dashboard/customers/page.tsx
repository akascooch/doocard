...
const customerSchema = z.object({
  ...
  barberId: z.coerce.number().min(1, "لطفا آرایشگر را انتخاب کنید"),
})
...
export default function CustomersPage() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ open: boolean; customerId: string | null }>({ open: false, customerId: null });
  ...
  return (
    <>
                      ...
                      return (
                        <FormItem>
                          <FormLabel>آرایشگر</FormLabel>
                           ...
                           >
                            <SelectTrigger>
                              <SelectValue placeholder={isBarber ? "شما" : "انتخاب آرایشگر..."} />
                            </SelectTrigger>
                            <SelectContent>
                              {barbers.map(barber => (
                ...
                sortedCustomers.map((customer) => (
                  <TableRow key={customer.id}>
                    ...
                    <TableCell>{customer.gender === 'MALE' ? 'مرد' : customer.gender === 'FEMALE' ? 'زن' : 'نامشخص'}</TableCell>
                                         <TableCell>
                       {(() => {
                         const barber = barbers.find(b => b.id === customer.barberId);
     ...
     {/* Confirmation Dialog */}
     <Dialog open={deleteConfirm.open} onOpenChange={(open) => setDeleteConfirm({ open, customerId: null })}>
       <DialogContent>
         <DialogHeader>
           <DialogTitle>تایید حذف</DialogTitle>
           <DialogDescription>
             آیا از حذف این مشتری اطمینان دارید؟ این عملیات قابل بازگشت نیست.
           </DialogDescription>
         </DialogHeader>
