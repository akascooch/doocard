          ...
          <Card>
            <CardHeader>
              <div className="flex flex-col gap-3">
                              ...
                              <SelectContent>
                                {accounts.filter(a => a.id !== transferForm.toAccountId).map(acc => (
                                  <SelectItem key={acc.id} value={String(acc.id)}>
                                    {acc.name} ({formatTomansFromRial(acc.balance)})
                                  </SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <Select value={String(transactionForm.categoryId || '')} onValueChange={(val) => setTransactionForm({...transactionForm, categoryId: parseInt(val)})}>
                              ...
                              <SelectContent>
                                {categories
                                  .filter(c => c.type === transactionForm.type && c.isActive !== false)
                                  .map(cat => (
                ...
                >
                  مجموع تراکنش‌های این لیست: {formatTomansFromRial(totalAmount)}
                </Badge>
              </div>
            </CardHeader>
            <CardContent>
