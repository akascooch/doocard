...
/** Browser: same-origin /api (Nginx → backend). Server SSR: internal backend only. */
function resolveServerApiBaseUrl(): string {
  const backend =
    process.env.BACKEND_URL ||
    process.env.INTERNAL_API_URL ||
    'http://127.0.0.1:3001';
  const origin = backend.replace(/\/$/, '').replace(/\/api$/, '');
  return `${origin}/api`;
}

export const API_BASE_URL =
  typeof window !== 'undefined' ? '/api' : resolveServerApiBaseUrl();
...
api.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    if (!isServer) {
      ...
      // Bearer token from login/refresh response body (access cookie is httpOnly)
    ...
    if (status === 401 && !originalRequest._retry && !isServer) {
      ...
      try {
        ...
        if (newAccessToken) {
          ...
          // Notify in-app providers (socket / notification center) to rebind
          // with the fresh access token. Does not weaken security — same token
          // the interceptor already stores for API calls.
          try {
            ...
            // ignore CustomEvent failures in non-browser contexts
          }
