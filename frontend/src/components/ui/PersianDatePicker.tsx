...
export default function PersianDatePicker({
  ...
  const handleChange = (date: Value) => {
    console.log('📅 [PersianDatePicker] handleChange called with:', date);
    setDateValue(date);
    
    if (date && typeof date === 'object' && 'format' in date) {
      const formatted = (date as DateObject).format(format);
      console.log('📅 [PersianDatePicker] Formatted date:', formatted);
      onChange(formatted);
    } else if (!date) {
      console.log('📅 [PersianDatePicker] Date cleared');
      onChange('');
    }
  };
  ...
  return (
    <div className={`space-y-2 ${className} ${disablePortal ? 'relative overflow-visible' : ''}`}>
      ...
      <DatePicker
        ...
        portal={portalTarget ? true : false}
