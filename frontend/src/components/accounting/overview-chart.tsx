  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        setError(null);
        console.log('📊 Fetching chart data for date range:', dateRange);
        
        // تبدیل تاریخ‌ها به فرمت مناسب برای API
        const fromDate = dateRange.from.toISOString().split('T')[0];
        const toDate = dateRange.to.toISOString().split('T')[0];
        
        const response = await api.get(`/accounting/chart?from=${fromDate}&to=${toDate}`);
        ...
        chartData = chartData.map((item: any) => {
          // اگر فیلد month وجود دارد، از آن استفاده کن
          const label = item.month || item.name || '';
          
          // اگر برچسب شامل روز هفته است (مثل "شنبه 3")
          if (label.includes('شنبه') || label.includes('یکشنبه') || label.includes('دوشنبه') || 
              label.includes('سه‌شنبه') || label.includes('چهارشنبه') || label.includes('پنج‌شنبه') || 
              label.includes('جمعه')) {
            return { 
              ...item, 
              name: label,
          ...
          // اگر برچسب نام ماه است
          const persianMonths = ['فروردین','اردیبهشت','خرداد','تیر','مرداد','شهریور','مهر','آبان','آذر','دی','بهمن','اسفند'];
          const jm = persianMonths.indexOf(label) + 1;
          return { 
            ...item, 
            name: label,
            jm,
        ...
        // استفاده مستقیم از داده‌های دریافتی از API
    ...
    <ResponsiveContainer width="100%" height={400}>
      ...
      >
        ...
        <Bar
          ...
          name="درآمد"
          onClick={(data) => {
            if (onMonthClick && data.jm) {
              onMonthClick(data.jm);
            }
          }}
          style={{ 
