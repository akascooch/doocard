              ...
              {withdrawals.map(w => {
                ...
                return (
                  <tr key={w.id} className="border-b">
                    ...
                    <td className="p-2">{formatTomansFromRial(w.amount)}</td>
          ...
          <Dialog open={withdrawDialog} onOpenChange={setWithdrawDialog}>
            <DialogContent>
              ...
              <form onSubmit={handleWithdraw} className="space-y-4">
                ...
                <div>موجودی: <b>{formatTomansFromRial(withdrawBarber?.balance || 0)}</b></div>
                <div>
                ...
                return (
                  <tr key={w.id} className="border-b">
                    ...
                    <td className="p-2 flex gap-2">
                      {w.status === 'PENDING' && (
                        <>
                          <Button size="sm" variant="default" onClick={() => openApproveDialog(w)} disabled={approveLoading === w.id}>
                            تایید و پرداخت
                          </Button>
                          <Button size="sm" variant="outline" onClick={() => handleReject(w.id)} disabled={rejectLoading === w.id}>
                      ...
                      <Button size="sm" variant="outline" onClick={() => setDeleteWithdrawalId(w.id)}>
                        حذف
                      </Button>
                    </td>
                  </tr>
          ...
          <Dialog open={approveDialog} onOpenChange={setApproveDialog}>
            <DialogContent>
              ...
              <form onSubmit={handleApproveFinal} className="space-y-4">
                <div>مبلغ: <b>{formatTomansFromRial(approveWithdrawal?.amount || 0)}</b></div>
                <div>
                ...
                [...salaries]
                  ...
                  .map((salary) => (
                    <TableRow key={salary.id}>
                      ...
                      <TableCell>{formatTomansFromRial(salary.amount)}</TableCell>
                      <TableCell>
                        <span
      ...
      <Dialog open={!!editWithdrawal} onOpenChange={() => setEditWithdrawal(null)}>
        <DialogContent>
          ...
          <form onSubmit={handleEditWithdrawal} className="space-y-4">
            <div>مبلغ فعلی: <b>{formatTomansFromRial(editWithdrawal?.amount || 0)}</b></div>
            <div>
      ...
      <Dialog open={!!deleteWithdrawalId} onOpenChange={() => setDeleteWithdrawalId(null)}>
        <DialogContent>
          ...
          <div className="flex gap-2 mt-4">
            <Button variant="outline" onClick={handleDeleteWithdrawal}>حذف</Button>
