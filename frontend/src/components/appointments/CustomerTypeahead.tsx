...
function looksLikeIranMobile(raw: string): boolean {
  return IRAN_MOBILE_RE.test(normalizeIranMobileClient(raw));
}
...
export default function CustomerTypeahead({
  ...
  const handleQuickAdd = async () => {
    ...
    const normalizedPhone = normalizeIranMobileClient(newCustomerForm.phone);
    ...
    try {
      ...
      if (useOffline) {
        ...
        const label = `${trimmedName} (${normalizedPhone})`;
      ...
      const preferredId =
        preferredEmployeeId != null && Number.isFinite(Number(preferredEmployeeId))
          ? Number(preferredEmployeeId)
          : null;

      const createdResponse = await api.post('/customers/quick', {
        name: trimmedName,
        phone: normalizedPhone,
        ...(preferredId != null && preferredId > 0
          ? { preferredEmployeeId: preferredId }
          : {}),
      });

      console.log('✅ Customer created:', createdResponse.data);
      ...
      toast({
        ...
        description: Array.isArray(apiMessage)
          ? apiMessage.join('، ')
          : apiMessage || 'ایجاد مشتری با خطا مواجه شد',
        variant: 'destructive',
      });
    }
  };
                  ...
                  <div className="p-4 text-center">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        const phonePrefill = looksLikeIranMobile(q)
                          ? normalizeIranMobileClient(q)
                          : '';
                        const namePrefill = phonePrefill ? '' : q;
                        setNewCustomerForm({
                          name: namePrefill,
                          phone: phonePrefill,
                          email: '',
